...
_LINKAGE = {'single', 'average', 'complete', 'weighted'}


def agglomerative_clustering(graph, affinity='weighted', linkage='weighted', f= lambda x: 1/float(x), check=True):
    ...
    if linkage == 'single':
        ...
        dendrogram = weighted_linkage_hierarchy(graph_copy, f)
...
def weighted_linkage_hierarchy(graph, f):
    remaining_nodes = set(graph.nodes())
    n_nodes = len(remaining_nodes)
